...
class DetailProduct {
    ...
    function getListProductGetCategory($category_id, $product_id) {
        ...
        $select = "WITH RECURSIVE CategoryCTE AS (
                    ...
                    SELECT products.title, products.description, products.id , discounts.discount_percent AS discountPercent
                    FROM products
                    LEFT JOIN discounts ON discounts.id = products.discount_id
                    WHERE category_id IN (
                    SELECT id
                    FROM CategoryCTE) AND products.id != $product_id AND products.deleted_at IS NULL
                    ORDER BY RAND()
                    LIMIT 15";
